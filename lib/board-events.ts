...
// Server-only pub/sub for board changes, backed by Postgres LISTEN/NOTIFY so it
// works across every server instance (notifications fan out through the DB).
// Mutations call pg_notify('board_task'); this module keeps one dedicated
// listener connection and forwards each notification to subscribed SSE streams.

type Listener = () => void;

type BoardEventState = {
...
const globalForBoard = globalThis as unknown as {
  __boardEvents?: BoardEventState;
};

const state: BoardEventState =
  globalForBoard.__boardEvents ??
  (globalForBoard.__boardEvents = {
    listeners: new Set(),
    client: null,
    connecting: null,
...
async function ensureListening(): Promise<void> {
  ...
  if (!state.connecting) {
    state.connecting = (async () => {
      ...
      await client.query("LISTEN board_task");
      state.client = client;
    })();
  }
  try {
