...
export type DashboardData = {
  ...
  /**
   * Every unavailable day from the start of this month on, which the calendar
   * needs in full: it pages between months on the client, so a map keyed by day
   * of the current month is not enough to draw the next one.
   */
  offEntries: { date: string; userId: string; userName: string }[];
...
export async function getDashboard(): Promise<DashboardData> {
  ...
  const [
    ...
    db
      ...
      .from(note)
      // Pinned first, the way /notes orders them, so what the team has been
  ...
  return {
    ...
    daysOff,
    offEntries: offRows.map((r) => ({
      date: r.date,
      userId: r.userId,
      userName: plainName(r.userName || r.email || "Member"),
    })),
