...
import { ARTICLES, CATEGORY_PILL, type Article } from "../../news-data";

// /v2/news/[slug] - the redesign's article detail, built from the
// "news-article-detail" Figma frame (node 25:200): the article card on the left
// with an info / related / tags rail on the right. Shell comes from
// app/v2/layout.tsx.
//
...
// articles instead: same category first, then the rest, capped at three.
...
export default async function V2ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const tags = article.tags ?? [article.category.toLowerCase()];
      ...
      <div className="flex items-start gap-[24px]">
        <article className="flex min-w-0 flex-1 flex-col gap-[24px] rounded-[12px] border border-[#243033]! bg-[#171e24] p-[40px]">
          <div className="flex flex-col gap-[16px]">
            <span
              className={`w-fit shrink-0 rounded-full px-[10px] py-[4px] text-[12px] font-bold ${CATEGORY_PILL[article.category]}`}
            >
              {article.category}
            </span>
            <h1 className="text-[24px] leading-[32px] font-bold text-[#e2e8f0]">{article.title}</h1>
            <div className="flex items-center gap-[16px]">
              <span className="flex items-center gap-[6px]">
                <Calendar size={14} strokeWidth={2} className="text-[#94a3b8]" />
                <span className="text-[13px] font-normal text-[#94a3b8]">{article.dateLong}</span>
              </span>
              <span className="text-[13px] font-normal text-[#64748b]">•</span>
              <span className="flex items-center gap-[6px]">
                <User size={14} strokeWidth={2} className="text-[#94a3b8]" />
                <span className="text-[13px] font-normal text-[#94a3b8]">{article.author}</span>
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-[20px] text-[14px]">
            {article.links ? (
              ...
              // excerpt stands in until articles come from the database.
              <p className="leading-[1.6] font-normal text-[#e2e8f0]">{article.excerpt}</p>
            )}
          </div>
        </article>
        ...
        <div className="flex w-[360px] shrink-0 flex-col gap-[16px]">
          <div className={`flex flex-col gap-[12px] ${CARD}`}>
            <p className="text-[14px] font-bold text-[#e2e8f0]">Article info</p>
          ...
          <div className={`flex flex-col gap-[12px] ${CARD}`}>
            <p className="text-[14px] font-bold text-[#e2e8f0]">Related articles</p>
            <div className="flex flex-col gap-[12px]">
              {related.map((r, i) => (
                <div key={r.slug} className="flex flex-col gap-[12px]">
                  <Link href={`/v2/news/${r.slug}`} className="flex flex-col gap-[6px]">
