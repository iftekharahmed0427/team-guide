...
// /v2/audits/[member] - one member's audits, from the "audit-member-detail"
...
export default async function V2AuditMemberPage({
  params,
}: {
    ...
    <div className="flex flex-col gap-[32px] p-[32px]">
      ...
      <div className="flex flex-col gap-[16px]">
        {member.audits.map((row) => (
          <Link
            key={row.id}
            href={`/v2/audits/${encodeURIComponent(member.key)}/${row.id}`}
