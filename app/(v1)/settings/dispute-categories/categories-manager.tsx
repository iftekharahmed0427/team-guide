...
import { createCategory, renameCategory, deleteCategory } from "@/lib/actions/dispute-categories";
...
export default function CategoriesManager({ categories }: { categories: Category[] }) {
  ...
  function add() {
    ...
    if (!name) return;
    run(() => createCategory(name), () => setDraft(""));
  }
  ...
  function remove(id: string, name: string) {
    if (!window.confirm(`Delete the "${name}" category? Existing disputes keep their label.`)) return;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="border border-border bg-surface">
        {categories.length === 0 ? (
          <p className="px-5 py-10 text-center text-sm text-muted">No categories yet. Add one above.</p>
        ) : (
          <ul>
            {categories.map((c) => (
              <li key={c.id} className="flex items-center gap-2 border-b border-border px-3 py-2 last:border-0">
                <input
                  value={names[c.id] ?? ""}
                  disabled={pending}
